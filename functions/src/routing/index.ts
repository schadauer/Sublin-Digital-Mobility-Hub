        ...
        try {
            ...
            if (data !== undefined && data['endId'] && data['startId'] && data['endAddress'] && data['startAddress']) {
                ...
                if (provider.length) {
                    ...
                    // Get route from station to endAddress
                    await writeRoute(publicSteps, sublinEndStep[0], context.params.userId, provider[0], {}, data['startId'], data['endId'], data['checkAddress']);
                } else {
                    // If no provider is available we provide information about the location
                    // Information about the postcode and the city
                    await writeRoute([], [], context.params.userId, {}, {}, data['startId'], data['endId'], data['checkAddress']);
                }
                return null;
            } else {
                return null;
